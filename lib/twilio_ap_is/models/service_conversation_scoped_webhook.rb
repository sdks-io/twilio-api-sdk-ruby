# twilio_ap_is
#
# This file was automatically generated by APIMATIC v3.0 (
# https://www.apimatic.io ).

require 'date'
module TwilioApIs
  # ServiceConversationScopedWebhook Model.
  class ServiceConversationScopedWebhook < BaseModel
    SKIP = Object.new
    private_constant :SKIP

    # A 34 character string that uniquely identifies this resource.
    # @return [String]
    attr_accessor :sid

    # The unique ID of the
    # [Account](https://www.twilio.com/docs/iam/api/account) responsible for
    # this conversation.
    # @return [String]
    attr_accessor :account_sid

    # The SID of the [Conversation
    # Service](https://www.twilio.com/docs/conversations/api/service-resource)
    # the Participant resource is associated with.
    # @return [String]
    attr_accessor :chat_service_sid

    # The unique ID of the
    # [Conversation](https://www.twilio.com/docs/conversations/api/conversation-
    # resource) for this webhook.
    # @return [String]
    attr_accessor :conversation_sid

    # The target of this webhook: `webhook`, `studio`, `trigger`
    # @return [String]
    attr_accessor :target

    # An absolute API resource URL for this webhook.
    # @return [String]
    attr_accessor :url

    # The configuration of this webhook. Is defined based on target.
    # @return [Object]
    attr_accessor :configuration

    # The date that this resource was created.
    # @return [DateTime]
    attr_accessor :date_created

    # The date that this resource was last updated.
    # @return [DateTime]
    attr_accessor :date_updated

    # A mapping from model property names to API property names.
    def self.names
      @_hash = {} if @_hash.nil?
      @_hash['sid'] = 'sid'
      @_hash['account_sid'] = 'account_sid'
      @_hash['chat_service_sid'] = 'chat_service_sid'
      @_hash['conversation_sid'] = 'conversation_sid'
      @_hash['target'] = 'target'
      @_hash['url'] = 'url'
      @_hash['configuration'] = 'configuration'
      @_hash['date_created'] = 'date_created'
      @_hash['date_updated'] = 'date_updated'
      @_hash
    end

    # An array for optional fields
    def self.optionals
      %w[
        sid
        account_sid
        chat_service_sid
        conversation_sid
        target
        url
        configuration
        date_created
        date_updated
      ]
    end

    # An array for nullable fields
    def self.nullables
      %w[
        sid
        account_sid
        chat_service_sid
        conversation_sid
        target
        url
        configuration
        date_created
        date_updated
      ]
    end

    def initialize(sid: SKIP, account_sid: SKIP, chat_service_sid: SKIP,
                   conversation_sid: SKIP, target: SKIP, url: SKIP,
                   configuration: SKIP, date_created: SKIP, date_updated: SKIP,
                   additional_properties: nil)
      # Add additional model properties to the instance
      additional_properties = {} if additional_properties.nil?

      @sid = sid unless sid == SKIP
      @account_sid = account_sid unless account_sid == SKIP
      @chat_service_sid = chat_service_sid unless chat_service_sid == SKIP
      @conversation_sid = conversation_sid unless conversation_sid == SKIP
      @target = target unless target == SKIP
      @url = url unless url == SKIP
      @configuration = configuration unless configuration == SKIP
      @date_created = date_created unless date_created == SKIP
      @date_updated = date_updated unless date_updated == SKIP
      @additional_properties = additional_properties
    end

    # Creates an instance of the object from a hash.
    def self.from_hash(hash)
      return nil unless hash

      # Extract variables from the hash.
      sid = hash.key?('sid') ? hash['sid'] : SKIP
      account_sid = hash.key?('account_sid') ? hash['account_sid'] : SKIP
      chat_service_sid =
        hash.key?('chat_service_sid') ? hash['chat_service_sid'] : SKIP
      conversation_sid =
        hash.key?('conversation_sid') ? hash['conversation_sid'] : SKIP
      target = hash.key?('target') ? hash['target'] : SKIP
      url = hash.key?('url') ? hash['url'] : SKIP
      configuration = hash.key?('configuration') ? hash['configuration'] : SKIP
      date_created = if hash.key?('date_created')
                       (DateTimeHelper.from_rfc3339(hash['date_created']) if hash['date_created'])
                     else
                       SKIP
                     end
      date_updated = if hash.key?('date_updated')
                       (DateTimeHelper.from_rfc3339(hash['date_updated']) if hash['date_updated'])
                     else
                       SKIP
                     end

      # Create a new hash for additional properties, removing known properties.
      new_hash = hash.reject { |k, _| names.value?(k) }

      additional_properties = APIHelper.get_additional_properties(
        new_hash, proc { |value| value }
      )

      # Create object from extracted values.
      ServiceConversationScopedWebhook.new(sid: sid,
                                           account_sid: account_sid,
                                           chat_service_sid: chat_service_sid,
                                           conversation_sid: conversation_sid,
                                           target: target,
                                           url: url,
                                           configuration: configuration,
                                           date_created: date_created,
                                           date_updated: date_updated,
                                           additional_properties: additional_properties)
    end

    def to_custom_date_created
      DateTimeHelper.to_rfc3339(date_created)
    end

    def to_custom_date_updated
      DateTimeHelper.to_rfc3339(date_updated)
    end

    # Provides a human-readable string representation of the object.
    def to_s
      class_name = self.class.name.split('::').last
      "<#{class_name} sid: #{@sid}, account_sid: #{@account_sid}, chat_service_sid:"\
      " #{@chat_service_sid}, conversation_sid: #{@conversation_sid}, target: #{@target}, url:"\
      " #{@url}, configuration: #{@configuration}, date_created: #{@date_created}, date_updated:"\
      " #{@date_updated}, additional_properties: #{@additional_properties}>"
    end

    # Provides a debugging-friendly string with detailed object information.
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} sid: #{@sid.inspect}, account_sid: #{@account_sid.inspect},"\
      " chat_service_sid: #{@chat_service_sid.inspect}, conversation_sid:"\
      " #{@conversation_sid.inspect}, target: #{@target.inspect}, url: #{@url.inspect},"\
      " configuration: #{@configuration.inspect}, date_created: #{@date_created.inspect},"\
      " date_updated: #{@date_updated.inspect}, additional_properties: #{@additional_properties}>"
    end
  end
end
